...
class ClassicGameRules:
  ...
  def win( self, state, game ):
    ...
    runtime = runtime_total - sum(list_of_time)
    print(f'runtime : {runtime}')
    list_of_time.append(runtime)
...
def replayGame( layout, actions, display ):
    import Agents, ghostAgents
    rules = ClassicGameRules()
    # If replaying, change the agent from ExpectimaxAgent to whatever agent with which you want to play
    agents = [Agents.ExpectimaxAgent()] + [ghostAgents.RandomGhost(i+1) for i in range(layout.getNumGhosts())]
...
def runGames( layout, pacman, ghosts, display, numGames, record, numTraining = 0, catchExceptions=False, timeout=30 ):
  ...
  if (numGames-numTraining) > 0:
    ...
    # print((f'Times: {list_of_time}'))
    print(f'Average time: {sum(list_of_time)/len(list_of_time)}')
...
if __name__ == '__main__':
  ...
  for item in sys.argv[:]:
    if item == 'ReflexAgent' or item == 'MinimaxAgent' or item == 'AlphaBetaAgent' or\
      item == 'ExpectimaxAgent':
      print(f'Agent: {item}')
      agent = item
  ...
  if agent == 'ReflexAgent':
    ...
    for ghost_type in sys.argv:
      if ghost_type == '-g':
        flag = False
        index = sys.argv.index(ghost_type)
        print(f'Ghost Agent : {sys.argv[index+1]}')
    if flag: 
      print('Ghost Agent : Random Ghost')

  flag = True
  for layout in sys.argv[:]:
    if layout == '-l':
      flag = False
      index = sys.argv.index(layout)
      print(f'Layout : {sys.argv[index+1]}')
  if flag:
